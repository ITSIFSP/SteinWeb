...
var options = {
    zoom: 10,
    center: { lat: -21.14691, lng: -48.94627 } //Catanduva - IFSP
};
...
function intervention() {
    ...
    interventionRef.on("value", function(snapshot) {
        ...
        var dateNow = moment().format("DD/MM/YYYY h:m");
        console.log(data);
        for (let index in data) {
            ...
            if (data[index].status == true || data[index].status == "true") {
                ...
                if (
                    moment(dateNow).isAfter(data[index].beginDate) &&
                    moment(dateNow).isBefore(data[index].endDate)
                ) {
                    //Entre intervalo
                    isInInterval = true;
                }
                if (
                    dateNow == data[index].beginDate ||
                    dateNow == data[index].endDate
                ) {
                    //Data Igual
                    isInInterval = true;
                }
